...
@section('form')
<form method="POST" action="{{ route('login.auth') }}" class="mx-5 py-4 bg-white border border-secondary-subtle rounded-3">
    @csrf
    ...
    <div class="form-content">
        ...
        <div class="text-danger">{{ $errors->first('failed') }}</div>
    </div>

    <div class="text-center my-3">
        <div>
            <button class="btn btn-info text-white" type="submit">ログイン</button>
        </div>
        
        <a href="{{ route('register.page') }}" class="link-offset-2 link-underline link-underline-opacity-0 d-block mt-3">アカウントをお持ちでない方はこちら</a>
    </div>
</form>
@endsection